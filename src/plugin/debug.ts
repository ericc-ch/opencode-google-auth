 ...
 */
export function startGeminiDebugRequest(
  meta: GeminiDebugRequestMeta,
): GeminiDebugContext | null {
  if (!debugEnabled) {
    return null
  }
  ...
  logDebug(
    `[Gemini Debug ${id}] Headers: ${JSON.stringify(maskHeaders(meta.headers))}`,
  )

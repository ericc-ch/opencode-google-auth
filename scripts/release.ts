...
const prevTagResult =
  await $`git describe --tags --abbrev=0 ${currentTag.trim()}^`
    .nothrow()
    .quiet()
const prevTag = prevTagResult.exitCode === 0 ? prevTagResult.text() : null

const logRange =
  prevTag ? `${prevTag.trim()}..${currentTag.trim()}` : currentTag.trim()
const format = "- %s (@%an)"
const commits =
  await $`git log ${logRange} --pretty=format:${format} --no-merges`.text()
